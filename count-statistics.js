...
function countStatistic(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        // let perProduct = data[i].product;
        if (obj[data[i].product]) {
            obj[data[i].product]["shoppers"].push(data[i].name);
            obj[data[i].product]["totalAmount"] += data[i].amount
            if (data[i].amount > obj[data[i].product]["maxAmount"]) {
                obj[data[i].product]["maxAmount"] = data[i].amount;
            }
            console.log(max);
            if (data[i].amount < obj[data[i].product]["minAmount"]) {
                obj[data[i].product]["minAmount"] = data[i].amount;
            }
        } else {
            obj[data[i].product] = {
                shoppers: [data[i].name],
