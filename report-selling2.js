...
function reportSelling(list) {
    ...
    for (let i = 0; i < list.length; i++) {
        let user = list[i];
        if(obj[user.brand]){
            obj[user.brand]["customers"].push(user.name)
            obj[user.brand]["sum"] += user.qty;
            if(obj[user.brand]["max"]<user.qty){
                obj[user.brand]["max"] = user.qty;
            }
            if(obj[user.brand]["min"]>user.qty){
                obj[user.brand]["min"] = user.qty;
            }
            obj[user.brand]["avg"] = obj[user.brand]["sum"] / obj[user.brand]["customers"].length
        } else {
            obj[user.brand] = {
                customers: [user.name],
                sum: user.qty,
